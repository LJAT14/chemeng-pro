// setup-writing.mjs - Run this to create the writing component folder
import fs from 'fs';
...
// Create writing component directory
const writingDir = 'src/components/writing';
if (!fs.existsSync(writingDir)) {
  fs.mkdirSync(writingDir, { recursive: true });
  log('✓ Created src/components/writing/', 'green');
}

// Create WritingChecker.jsx
const writingCheckerContent = `import React, { useState } from 'react';
...
fs.writeFileSync(path.join(writingDir, 'WritingChecker.jsx'), writingCheckerContent);
log('✓ Created WritingChecker.jsx', 'green');

// Create businessPhrases.js data file
const dataDir = 'src/data';
const businessPhrasesContent = `export const businessPhrases = {
  emails: {
    openings: [
      {
        situation: 'First contact - Formal',
        phrases: [
          { en: 'I am writing to inquire about...', pt: 'Escrevo para perguntar sobre...' },
          { en: 'I would like to introduce myself...', pt: 'Gostaria de me apresentar...' }
        ]
      }
    ],
    closings: [
      {
        situation: 'General closing',
        phrases: [
          { en: 'Best regards', pt: 'Atenciosamente' },
          { en: 'Kind regards', pt: 'Cordialmente' }
        ]
      }
    ]
  },
...
fs.writeFileSync(path.join(dataDir, 'businessPhrases.js'), businessPhrasesContent);
log('✓ Created businessPhrases.js', 'cyan');

log('\n✅ Setup complete!', 'green');
log('\nNext steps:', 'blue');
log('1. Copy grammar rules and vocabulary from artifacts above');
log('2. Run: npm run dev');
log('3. Test the Writing Studio page');